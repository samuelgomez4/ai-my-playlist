...
export async function generatePrompt(options: PromptGenerationOptions) {
  ...
  try {
    const { text } = await generateText({
      ...
      Some of the possibilities that the assistant could answer here are: "Create a playlist with more songs of reggaeton", "Add more songs of the album Vida", "Add more songs from the 2010's", etc. 
      ...
      -Create New Playlist Based on Existing: "Create a new playlist with other songs from DJ Snake", "Create a playlist with the remix versions of the songs in the playlist if they exist", "Create a new playlist with different versions of the songs if they exist", etc. 
      -Delete Songs from Existing Playlist: 'Delete all the songs that are less than 3 min long", 'Delete all the songs from Luis Fonsi', 'Delete all the songs that were released before 2018', 'Delete all the songs that have only one word in the title', 'Delete all the songs from the album "Vida"', etc. 
    ...
    return {
      ok: false,
      message: 'There was an error. You may have reached the rate limit. Try again later.',
    };
  }
}
