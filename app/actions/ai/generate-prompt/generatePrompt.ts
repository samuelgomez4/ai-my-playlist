...
import { safetySettings, smallModel } from '../config';
...
export async function generatePrompt(options: Options) {
  try {
    const { text } = await generateText({
      model: google(smallModel, { safetySettings: [safetySettings] }),
      system: `You are an assistant that is used to create a prompt for another assistant that receives an instruction to create a new playlist either from scratch or having another playlist as context. Your task is to respond with a prompt. You're gonna receive an object with 2 properties one is the action that the user wants to do (the available actions are: "Create Playlist from Scratch" | "Create Playlist from Selected Playlist" | "Create New Playlist Based on Existing" | "Add New Songs to Existing Playlist" | "Delete Songs from Existing Playlist") and the other one is a list of songs that the AI will use to do the action. Your task is to grab the action and the list of songs to create a prompt. If no action is provided just choose a random from the ones available. If an empty list of songs is provided then create the prompt based only on the "create playlist from scratch" action. The shape of each one of the internal list is the following: [name, artists[], album, duration with the format minutes:seconds, releaseDate, addedByUserAt]. You CANNOT reply with something that is not the prompt related with creating a new list or edit the list. For example:
